...
export default function DetalhesDemandaPage() {
    ...
    const [fotoAtiva, setFotoAtiva] = useState(0); // Controle do Carrossel

    // NOVOS ESTADOS PARA O MODAL DE FINALIZAÇÃO
    const [isModalFinalizarOpen, setIsModalFinalizarOpen] = useState(false);
    const [salvandoStatus, setSalvandoStatus] = useState(false);

    useEffect(() => {
        async function buscarDemanda() {
            // Trava de segurança: se não tiver ID na URL, para de carregar.
            if (!demandaId) {
                setCarregando(false);
                return;
            }
    ...
    const handleFinalizarDemanda = async (motivo: "na_plataforma" | "fora_plataforma") => {
        ...
        try {
            ...
            // Atualiza a tela em tempo real sem precisar recarregar a página
            setDemanda((prev: any) => ({
                ...prev,
                status: "resolvida",
                motivoFechamento: motivo
            ...
            setIsModalFinalizarOpen(false); // Fecha o modal
        } catch (error) {
                    ...
                    {carregando ? (
                        ...
                        <>
                            ...
                            <div className="bg-white rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.05)] p-8 md:p-10">
                                                ...
                                                <div className="flex gap-3 overflow-x-auto py-2 max-w-lg w-full justify-center scrollbar-hide">
                                                    {demanda.fotos.map((foto: string, index: number) => (
                                                        <button
                                                            ...
                                                            className={`relative w-20 h-20 rounded-md overflow-hidden border-2 transition-all flex-shrink-0 ${fotoAtiva === index
                                ...
                                {demanda.documentoPdf && (
                                    <div className="mb-8">
                                        ...
                                        <a
                                            ...
                                            className="inline-flex items-center gap-3 bg-white border border-gray-200 text-gray-700 hover:bg-gray-50 hover:border-gray-300 transition-all rounded-lg py-3 px-4 shadow-sm"
                                        >
                                ...
                                {/* Botão de Ação Dinâmico (Só aparece se estiver aberta) */}
                                <div className="flex justify-center pt-6 border-t border-gray-100 mt-8">
                                    {demanda.status === "aberta" ? (
                                        <button
                                            onClick={() => setIsModalFinalizarOpen(true)}
                                            className="bg-gray-900 hover:bg-black text-white font-medium py-3 px-10 rounded-lg transition-colors shadow-md"
                                        >
                            ...
                            <div className="bg-white rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.05)] p-8 md:p-10">
                                <h2 className="text-xl font-bold text-gray-900 mb-6">Fornecedores Interessados</h2>
                                <div className="w-full bg-pedraum-dark rounded-xl overflow-hidden shadow-lg">
                                    <div className="overflow-x-auto">
                                        <table className="w-full text-left border-collapse min-w-[600px]">
                                            <thead>
                                                <tr className="border-b border-gray-700/50 bg-black/20">
                                                    <th className="py-4 px-6 font-medium text-gray-400 text-sm uppercase tracking-wider">Nome</th>
                                                    <th className="py-4 px-6 font-medium text-gray-400 text-sm uppercase tracking-wider">Email</th>
                                                    <th className="py-4 px-6 font-medium text-gray-400 text-sm uppercase tracking-wider">Telefone</th>
                                                </tr>
                                            </thead>
                                            <tbody className="text-gray-100">
                                                <tr className="border-b border-gray-700/50 hover:bg-white/5 transition-colors">
                                                    <td className="py-4 px-6 font-medium">Lucas David (Exemplo)</td>
                                                    <td className="py-4 px-6 text-gray-300">lucas@email.com</td>
                                                    <td className="py-4 px-6 text-gray-300">(35) 99999-9999</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                ...
                                <p className="text-xs text-gray-400 mt-4 text-center">
                                    *A lista real será exibida assim que os fornecedores começarem a enviar propostas.
                                </p>
                            </div>
                        </>
                    )}
                ...
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl p-6 md:p-8 animate-in fade-in zoom-in duration-200">
                        ...
                        <div className="space-y-3 mb-8">
                            <button
                                onClick={() => handleFinalizarDemanda("na_plataforma")}
                                disabled={salvandoStatus}
                                className="w-full text-left p-4 rounded-xl border-2 border-gray-100 hover:border-pedraum-orange hover:bg-orange-50 transition-all group disabled:opacity-50"
                            >
                            ...
                            <button
                                onClick={() => handleFinalizarDemanda("fora_plataforma")}
                                disabled={salvandoStatus}
                                className="w-full text-left p-4 rounded-xl border-2 border-gray-100 hover:border-gray-300 hover:bg-gray-50 transition-all disabled:opacity-50"
                            >
